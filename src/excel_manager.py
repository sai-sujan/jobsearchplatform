    ...
    def update_master_list(self, new_jobs: List) -> int:
        ...
        for job in new_jobs:
            new_data.append({
                ...
                'Premium_Indicators': job.premium_indicators,
                'Search_Query': job.search_query,
                'Job_Description': job.job_description[:15000] if job.job_description else '',  # Limit length
                'Date_Added': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                'Keywords_Matching_Score': 0, # Default
                'Analysis_File': '',
                'Analysis_JSON': ''
            })
        ...
        # Ensure new_df has all standard columns (though we just created it with them mostly)
        ...
        if existing_df.empty:
            ...
            # Ensure existing_df has all strict columns too
            for col in self.STANDARD_COLUMNS:
