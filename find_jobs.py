...
def main():
    ...
    skill_matcher = SkillMatcher()  # Initialize skill matcher
    ...
    for role_name, keywords, count in SEARCH_QUERIES:
        ...
        scraper.close_browser()  # Clean up after each search
        ...
        for job in jobs:
            ...
            if not passed:
                # Still add rejected jobs for reference
                verdict = "NO"
                ...
                matched_skills = ", ".join(score_result['matched_skills'][:10])  # Top 10
                missing_skills = ", ".join(score_result['missing_skills'][:10])   # Top 10
    ...
    if all_new_jobs:
        ...
        print(f"  • Passed filters (YES): {passed_filter} ({int(passed_filter/total_new*100)}%)")
        print(f"  • Good matches (70%+): {good_matches} ({int(good_matches/total_new*100)}%)")
        print(f"  • Perfect matches (90%+): {perfect_matches} ({int(perfect_matches/total_new*100)}%)")
    else:
